...
void server_init(UA_Server *server) {
  ...
  for (size_t idx = 0; idx < number_of_config; ++idx) {
    ...
    sensor_obj_attr.displayName =
        config::sensor_config_array_ast[idx].sensor_obj.displ_name;
    sensor_obj_attr.description =
        config::sensor_config_array_ast[idx].sensor_obj.description;

    UA_Server_addObjectNode(
        server, config::sensor_config_array_ast[idx].sensor_obj.id,
        UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
        UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
        config::sensor_config_array_ast[idx].sensor_obj.qualified_name,
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEOBJECTTYPE), sensor_obj_attr, NULL,
        NULL);

    // Sensor name/type
    UA_VariableAttributes sensor_name_attr = UA_VariableAttributes_default;
    UA_String sensor_name_str = UA_STRING_ALLOC(
        config::sensor_config_array_ast[idx].sensor_name.c_str());
    UA_Variant_setScalar(&sensor_name_attr.value, &sensor_name_str,
                         &UA_TYPES[UA_TYPES_STRING]);
    sensor_name_attr.displayName = config::sensor_config_array_ast[idx]
                                       .sensor_child.sensor_type.displ_name;
    sensor_name_attr.description = config::sensor_config_array_ast[idx]
                                       .sensor_child.sensor_type.description;
    ...
    UA_Server_addVariableNode(
        ...
        config::sensor_config_array_ast[idx]
            .sensor_child.sensor_type.qualified_name,
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), sensor_name_attr,
        NULL, NULL);

    // data + time stamp
    UA_VariableAttributes data_timestamp_attr = UA_VariableAttributes_default;
    UA_String data_time_stamp_init = UA_STRING("");
    UA_Variant_setScalar(&data_timestamp_attr.value, &data_time_stamp_init,
                         &UA_TYPES[UA_TYPES_STRING]);
    data_timestamp_attr.displayName =
        config::sensor_config_array_ast[idx]
            .sensor_child.data_timestamp.displ_name;
    data_timestamp_attr.description =
        config::sensor_config_array_ast[idx]
            .sensor_child.data_timestamp.description;
    UA_Server_addVariableNode(
        server,
        ...
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        config::sensor_config_array_ast[idx]
            .sensor_child.data_timestamp.qualified_name,
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
        data_timestamp_attr, NULL, NULL);

    // sensor object
    UA_ObjectAttributes signature_obj_attr = UA_ObjectAttributes_default;
    signature_obj_attr.displayName =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_object.displ_name;
    signature_obj_attr.description =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_object.description;

    UA_Server_addObjectNode(
        server,
        config::sensor_config_array_ast[idx].sensor_child.signature_object.id,
        config::sensor_config_array_ast[idx]
            .sensor_obj.id,  // -> Parent shall be the object sensor
        UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_object.qualified_name,
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEOBJECTTYPE), signature_obj_attr, NULL,
        NULL);

    // signature algorithm name
    UA_VariableAttributes signature_algo_name_attr =
        UA_VariableAttributes_default;
    UA_String sig_algo_name_str = UA_STRING_ALLOC(
        config::sensor_config_array_ast[idx].signature_algorithm_name.c_str());
    UA_Variant_setScalar(&signature_algo_name_attr.value, &sig_algo_name_str,
                         &UA_TYPES[UA_TYPES_STRING]);
    signature_algo_name_attr.displayName =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_name.displ_name;
    signature_algo_name_attr.description =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_name.description;

    UA_Server_addVariableNode(
        server,
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_name.id,
        config::sensor_config_array_ast[idx].sensor_child.signature_object.id,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_name.qualified_name,
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
        signature_algo_name_attr, NULL, NULL);

    // signature length
    UA_VariableAttributes signature_length_attr = UA_VariableAttributes_default;
    UA_UInt16 sig_length = CRYPTO_BYTES;
    UA_Variant_setScalar(&signature_length_attr.value, &sig_length,
                         &UA_TYPES[UA_TYPES_UINT16]);
    signature_length_attr.displayName =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_length.displ_name;
    signature_length_attr.description =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_length.description;

    UA_Server_addVariableNode(
        server,
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_length.id,
        config::sensor_config_array_ast[idx].sensor_child.signature_object.id,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_length.qualified_name,
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
        signature_length_attr, NULL, NULL);

    // signature byte buffer
    UA_VariableAttributes signature_byte_buffer_attr =
        UA_VariableAttributes_default;
    signature_byte_buffer_attr.displayName =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_buffer.displ_name;

    signature_byte_buffer_attr.description =
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_buffer.description;
    std::vector<uint8_t> tmpBuffer(
        config::sensor_config_array_ast[idx].signature_len, 0);

    UA_Variant_setArray(&signature_byte_buffer_attr.value, tmpBuffer.data(),
    ...
    UA_Server_addVariableNode(
        server,
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_buffer.id,
        config::sensor_config_array_ast[idx].sensor_child.signature_object.id,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        config::sensor_config_array_ast[idx]
            .sensor_child.signature_child.signature_buffer.qualified_name,
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
        signature_byte_buffer_attr, NULL, NULL);
  }
}
}  // namespace server_lib
...
void fetch_data(size_t config_index, std::string input_data) {
  ...
  if (config_index > number_of_config) {
    ...
    config::sensor_config_array_ast[config_index].data_and_timestamp =
        input_data;
  }
}

/**
 * @brief Patch the signature from the sensor -> Severlib, shall be called by
 * sensor
 *
