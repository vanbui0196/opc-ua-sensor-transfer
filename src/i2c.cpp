 ...
 */
void IO_ERROR_REPORT_COND(bool cond, std::string message, std::function<void(void)> callback_fn)
{
    if (true == cond)
    {
        // Printout the error
...
float median_filter(float values[], int count)
{
    ...
    // Simple bubble sort. No need to call the O(n * log(n)) of the function STL,only 40 samples
...
/**
 * @brief reading sensor value from the i2c sensor. This is 100% convert from the python file
 * of the predecessor owner, i don't have any datasheet of the sensor
 *
...
float read_sensor_data(int file)
{
    ...
    if (valid_samples > 0)
    {
        float result = median_filter(samples, valid_samples);
        return result;
    }
        ...
        while (i2c_running_flg)
        {
            ...
            // Update global data with thread safety
            {
                ...
                if (speed >= 0)
                {
                    ...
                    std::copy_n(sigOut.data(), CRYPTO_BYTES, i2c_shared_data.signature.data());
