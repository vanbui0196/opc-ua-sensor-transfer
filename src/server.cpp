...
#include "opcua_i2c.h"
#define OPCUA_SERVER_CALLBACK_TIME 4000          // 4 seconds in milliseconds
 ...
 * According to the mode (EVENT_DRIVEN or CALLBACK mode), then the server data structure
 * will update according.
 */
static bool update_data_from_i2c(void)
{
...

    {
        ...
        if (is_valid_flg)
        {
            if (operation_mode == MODE_EVENT_DRIVEN)
            {
                ...
                /**
                    NOTE: data in the event-driven mode only update when current data (read from i2c) > last_data.
                    This mode is designed for saving the traffic of network, only sending data when needed.
                */
                ...
                if (true == first_sample || i2c_data.current_data > previous_data)
                {
                    ...
                    // since the first sample the current data is 0, data working must always work
                ...
                else
                {
                    ...
                    // log the value during the development -> will be delete after finish development
                    UA_LOG_INFO(UA_Log_Stdout,
                                UA_LOGCATEGORY_SERVER,
                                "[EVENT DRIVEN MODE]: sensor data not increase %.2f < %.2f\n",
            ...
            else
            {
                ...
                // copy the signature from i2c-device driver -> the server server
                memcpy(OpcUa_Server_globData_st.signatureBuf_au8,
                       i2c_data.signature.data(), CRYPTO_BYTES);
...
/**
 * @brief This sub-API is used for updating the nodes (string + signature) incase of
 * event-driven mode
 *
 * @param server pointer to the server
 ...
 */
static void trigger_data_increasing_event(UA_Server *server, UA_Float current_value,
                                          UA_Float previous_value)
{
    // create the new event with the pre-defined type
    UA_StatusCode retval = UA_Server_createEvent(server, value_increasement_event_typeid,
                                                 &value_increasement_event_id);
...
static void update_opcua_nodes(UA_Server *server, std::string mode_name)
{
    ...
    */
    ...
    UA_String data_time_string = UA_STRING_ALLOC(OpcUa_Server_globData_st.data_and_timestamp.c_str());
    UA_Variant_setScalar(&data_time_variant, &data_time_string, &UA_TYPES[UA_TYPES_STRING]);
    UA_Server_writeValue(server, OpcUa_rawData_String_NodeIdSt, data_time_variant);
    ...
    UA_Server_writeValue(server, OpcUa_sensorSignatureId_NodeIdSt, signature_buffer);
    /*
    ----------------------------
     LOG: Debug purpsoe
    ----------------------------
    */
    std::string log_data = std::string(reinterpret_cast<char *>(data_time_string.data), data_time_string.length);
    ...
    -----------------------------------------------------
    */
    if (operation_mode == MODE_EVENT_DRIVEN && true == OpcUa_Server_globData_st.should_update_nodes)
    {
        trigger_data_increasing_event(server, OpcUa_Server_globData_st.current_data, previous_data);
    }
...
static void update_sendor_data_event_mode(UA_Server *server, void *data)
{
    ...
    if (is_valid_data)
    {
        ...
        // actually, this is check is not required since the check already performed
        // in the beginning of update_opcua_nodes
        if (should_update)
        {
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (operation_mode == MODE_CALLBACK)
    {
        ...
        std::cout << "Setting up callback mode each" << OPCUA_SERVER_CALLBACK_TIME << "(ms)" << std::endl;
    ...
    */
    ...
    ----------------------------------------------
        clean up:
            1. Signal handler of SIGINT or SIGTERM with the case of UA_Server_run
