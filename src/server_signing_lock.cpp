...

  {
    ...
    if (is_valid_flg) {
      ...
      if (operation_mode == MODE_EVENT_DRIVEN) {
        ...
        /**
            NOTE: data in the event-driven mode only update when current data
           (read from i2c) > last_data. This mode is designed for saving the
           traffic of network, only sending data when needed.
        */
        ...
        if (true == first_sample || i2c_data.current_data > previous_data) {
          ...
          // since the first sample the current data is 0, data working must
          // always work
          ...
          // log the value during the development -> will be delete after finish
          // development
          UA_LOG_INFO(
